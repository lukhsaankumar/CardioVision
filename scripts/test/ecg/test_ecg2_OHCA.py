...
def load_ohca_segments(json_path, window_size=250, stride=125, target_fs=250, label=2):
    try:
        ...
        sampling_frequency = data['samplingFrequency']  # Expected: 512 Hz
...
def test_ohca_ecg_model():
    ...
    window_size = 250  # At 512 Hz, ~0.488 seconds
    stride = 125       # Overlap to increase sample count
    target_fs = 250    # Match LSTM training data
    high_risk_label = 2
    ...
    try:
        ...
        print("❌ Model file '../CardioVision/models/ecg/lstm_model_multiclass.pth' not found.")
        return
    model.to(device)
    model.eval()
    ...
    for class_name in class_names:
        ...
        if segments:
            for segment in segments[:10]:  # Limit to first 10 for brevity
